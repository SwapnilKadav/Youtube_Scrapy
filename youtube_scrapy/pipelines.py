...
class YoutubeScrapyPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if valid:
            if self.collections.count_documents(item)!=0:
                self.collections.update_one(filter,{"$set":{'Comments':item['Comments']}})
                self.collections.update_one(filter,{"$set":{'Transcript':item['Transcript']}})
                self.collections.update_one(filter,{"$set":{'Title':item['Title']}})
                self.collections.update_one(filter,{"$set":{'Description':item['Description']}})
            else:
                self.collections.insert_one(dict(item))
                return item
        
        # filter = {"URL" :item['URL']}
        # print(filter,"++++++++++++++++++++++++=", self.collections.count_documents(item) )
        # if self.collections.count_documents(item) != 0:
        #     self.collections.update_one(filter, {'$set': {"Comments" :item['Comments']}})
        # else:
        #     self.collections.insert_one(dict(item))
